...
func TestDefaultRemoteConfigurationPropagatesActiveW3CTrace(t *testing.T) {
	...
	ctx := trace.ContextWithSpanContext(t.Context(), trace.NewSpanContext(trace.SpanContextConfig{TraceID: traceID, SpanID: spanID, TraceFlags: trace.FlagsSampled}))
	header := remote.OpenTelemetryContextHeaders(ctx)
	if header.Get("traceparent") != "00-4bf92f3577b34da6a3ce929d0e0e4736-00f067aa0ba902b7-01" {
		t.Fatalf("traceparent=%q", header.Get("traceparent"))
	}
}
...
func TestFactoryExchangesAndCachesIdentityServiceToken(t *testing.T) {
	...
	binding, err := remote.NewFactory(remote.Config{BaseURL: server.URL, ServiceTokens: source, HTTPClient: server.Client()}).Open(t.Context(), notificationsdk.ApplicationRef{WorkspaceID: "workspace", ApplicationKey: "runtime"})
	if err != nil {
		t.Fatal(err)
	}
	intent := contract.NotificationIntent{ID: "event", WorkspaceID: "workspace", SourceEventID: "source", EventType: "report.completed", RecipientUserIDs: []string{"user"}, OccurredAt: "2026-08-28T00:00:00.000000000Z", SubjectType: "report", SubjectID: "report", SubjectVersion: "one"}
...
func TestRemoteSystemTemplatesUseOnlyServiceAuthority(t *testing.T) {
	...
	defer server.Close()
	binding, err := remote.NewFactory(remote.Config{BaseURL: server.URL, ServiceCredential: "service-secret", HTTPClient: server.Client()}).Open(t.Context(), notificationsdk.ApplicationRef{WorkspaceID: "workspace", ApplicationKey: "runtime"})
	if err != nil {
		t.Fatal(err)
	}
