...
func TestRemoteFactoryContract(t *testing.T) {
	...
	contracttest.Run(t, func(testing.TB) (notificationsdk.Factory, notificationsdk.ApplicationRef) {
		return remote.NewFactory(remote.Config{BaseURL: server.URL, ServiceCredential: "service", HTTPClient: server.Client()}), notificationsdk.ApplicationRef{TenantID: "tenant", WorkspaceID: "workspace", ApplicationKey: "runtime"}
	})
}
